...
CONTAINER_FOLDER=containers/tc-submissions/submission-python-stream/code
...
DATASET_ID=bifurcating

mkdir $OUTPUT_FOLDER/$DATASET_ID
(time docker run $DATA_MOUNT $OUTPUT_MOUNT $IMAGE /data/inputs/${DATASET_ID}.h5 /outputs/${DATASET_ID}/) &> $OUTPUT_FOLDER/$DATASET_ID/time.txt
...
sh scripts/runner_stream.sh $DATA_FOLDER $OUTPUT_FOLDER $IMAGE
